...
const Mint = ({ provider, guesses, isGameWon }) => {
  ...
  const generateEmojiGrid = (guesses, tiles) => {
  ...
  const mintToken = async (medadataURI) => {
  ...
  async function storeImageAndMetadata(key, buffer) {
    if (process.env.REACT_APP_FLEEK_SECRET && process.env.REACT_APP_FLEEK_KEY) {
      try {
              ...
              {success && (
                ...
                >
                  <p className="block sm:inline">
                    ...
                    it in our{" "}
                    <a href={process.env.REACT_APP_OPENSEA_COLLECTION} target="_blank">
                      collection
                    </a>{" "}
                    or mint another one.
                  </p>
